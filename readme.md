...
- Sometimes it is useful to not run the query. For example if we know the user is not logged in, I don't want to run the query in `useFindMany`. Since you're not supposed to conditionally render hooks in react, it would be nice if there was a way to disable the query altogether, and just return `null` for the data.
